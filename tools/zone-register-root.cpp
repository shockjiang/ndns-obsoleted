...
/*
 * This program register the root zone into the NDNS
 * @attention:
 ...
 *    2) NDN system: rrsets table add one entry (ID-CERT)
 *
 *
 *
 */
...
main (int argc, char * argv[])
{
  ...
  try {
    ...
    hidden.add_options () ("cert,c", po::value<string> (&certPath),
  ...
  if (!keyChain.doesIdentityExist (root)) {
    cout << root.toUri () << " is not an identity in the KeyChain System" << endl;
    return 0;
  }
